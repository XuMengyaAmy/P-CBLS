...
class COCO_SPL(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, easy_ratio=1.0): # removed id_root=None, 
        
        roots = {}
        roots['train'] = {
            'img': img_root,
            'cap': os.path.join(ann_root, 'sorted_captions_train.json')  
        }
        roots['val'] = {
            'img': img_root,
            'cap': os.path.join(ann_root, 'captions_val.json')
        ...
        with nostdout():
            self.train_examples, self.val_examples = self.get_samples(roots, easy_ratio)
        examples = self.train_examples + self.val_examples
        super(COCO_SPL, self).__init__(examples, {'image': image_field, 'text': text_field})   
    ...
    @classmethod
    # def get_samples(cls, roots):
    def get_samples(cls, roots, easy_ratio):
        train_samples = []
        val_samples = []
        ...
        for split in ['train', 'val']:
            ...
            for index in range(len(anns)):
                ...
                if split == 'train':
                    train_samples.append(example)
                    # =============== SPL related code ================= #
                    if len(train_samples) == int( len(anns) * easy_ratio ): # when easy_ratio =1, it's normal loading and will load all samples
                        break
                    # =================================================== #
