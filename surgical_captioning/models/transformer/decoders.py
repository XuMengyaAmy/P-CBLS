...
class MeshedDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=512, dropout=.1,
        ...
        self.register_state('running_mask_self_attention', torch.zeros((1, 1, 0)).bool()) # original one is .byte(), .bool() 
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        mask_self_attention = mask_self_attention + (input == self.padding_idx).unsqueeze(1).unsqueeze(1).bool()  # original one is .byte()  
        ...
        # print('after word pos emb')
        # print(out.shape) # torch.Size([50, 16, 512])
